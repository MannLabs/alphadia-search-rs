...
def main():
    parser = argparse.ArgumentParser(description="Analyse candidate features with target/decoy histograms")
    parser.add_argument(
        "--features_path",
    ...
    logger.info(f"Targets: {(df['decoy'] == 0).sum():,}, Decoys: {(df['decoy'] == 1).sum():,}")
