...
/// A library from `(precursor_idx, precursor m/z, fragments as (m/z, intensity))`.
fn library(precursors: &[(usize, f32, &[(f32, f32)])]) -> SpecLibFlat {
    ...
    SpecLibFlat::from_vecs(
        ...
        // Fragment number 2, so the y1 filter of the scorer keeps every fragment.
        vec![2; n_fragments],
        vec![2; n_fragments],
...
fn test_two_candidates_sharing_three_fragments_compete() {
    ...
    let features = compute_context_features(&dia_data, &lib, &candidates, &params(3, 1)).unwrap();
...
fn test_min_shared_threshold_separates_matches_from_competitors() {
    ...
    let features = compute_context_features(&dia_data, &lib, &candidates, &params(4, 1)).unwrap();
...
fn test_cycle_radius_limits_the_comparison() {
    ...
    let within = compute_context_features(&dia_data, &lib, &candidates, &params(3, 1)).unwrap();
    assert_eq!(row(&within, 1, 0).n_competitors, 1.0);
    assert_eq!(row(&within, 1, 0).candidate_density, 1.0);

    let outside = compute_context_features(&dia_data, &lib, &candidates, &params(3, 0)).unwrap();
...
fn test_fragments_within_tolerance_match_across_bins() {
    ...
    let matched = compute_context_features(&dia_data, &lib, &candidates, &params(3, 1)).unwrap();
    assert_eq!(row(&matched, 1, 0).shared_frac_any, 1.0);
    assert_eq!(row(&matched, 1, 0).n_competitors, 1.0);

    let mut narrow = params(3, 1);
    narrow.mass_tolerance = 5.0;
...
fn test_empty_candidates_give_empty_result() {
    ...
    let features =
        compute_context_features(&dia_data, &lib, &CandidateCollection::new(), &params(3, 1))
            .unwrap();
...
fn test_invalid_parameters_are_rejected() {
    ...
    let mut zero_tolerance = params(3, 1);
    zero_tolerance.mass_tolerance = 0.0;
    assert!(compute_context_features(&dia_data, &lib, &candidates, &zero_tolerance).is_err());

    assert!(compute_context_features(&dia_data, &lib, &candidates, &params(0, 1)).is_err());

    let mut no_fragments = params(3, 1);
    no_fragments.top_k_fragments = 0;
    assert!(compute_context_features(&dia_data, &lib, &candidates, &no_fragments).is_err());
}
