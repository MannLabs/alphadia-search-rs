...
impl OptimizedDIADataBuilder {
    pub fn from_alpha_raw(alpha_raw_view: &AlphaRawView) -> DIADataNextGen {
        ...
        let quadrupole_observations = Self::build_observations_rayon_parallel(
            alpha_raw_view, &mz_index
        );
    ...
    fn build_observations_rayon_parallel(
        ...
        let max_delta_scan_idx = alpha_raw_view.spectrum_delta_scan_idx
            .iter()
            .max()
            .copied()
            .unwrap_or(0);
        ...
        let observations: Vec<QuadrupoleObservationNextGen> = (0..=max_delta_scan_idx)
            .into_par_iter()
            .map(|delta_scan_idx| {
                Self::build_single_observation_rayon(
                    alpha_raw_view, mz_index, delta_scan_idx
                )
            })
            .collect();
    ...
    fn build_single_observation_rayon(
        ...
        let matching_spectra: Vec<usize> = alpha_raw_view.spectrum_delta_scan_idx
            .iter()
            .enumerate()
        ...
        if matching_spectra.is_empty() {
            ...
            let mut empty_obs = QuadrupoleObservationNextGen::new_with_capacity(
                [0.0, 0.0], 0, mz_index.len(), 0
            );
