...
impl DIADataBuilder {
    pub fn from_alpha_raw(alpha_raw_view: &AlphaRawView) -> DIAData {
        ...
        let num_quadrupole_observations =
            alpha_raw_view.spectrum_delta_scan_idx.iter().max().unwrap() + 1;
    ...
    fn build_quadrupole_observation(
        ...
        for i in 0..alpha_raw_view.spectrum_delta_scan_idx.len() {
            if alpha_raw_view.spectrum_delta_scan_idx[i] == delta_scan_idx {
                ...
                let mz_slice = alpha_raw_view
                    .peak_mz
                    .slice(s![peak_start_idx..peak_stop_idx]);
                let intensity_slice = alpha_raw_view
                    .peak_intensity
                    .slice(s![peak_start_idx..peak_stop_idx]);
