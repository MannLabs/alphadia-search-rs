...
Candidates must be pre-filtered with `num_over_50 >= 3` **before** passing to the classifier. This hard filter is not performed inside the classifier — it is a candidate selection concern handled upstream.

```python
df = df[df["num_over_50"] >= 3]
```
