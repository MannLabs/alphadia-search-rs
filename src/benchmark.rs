...
pub fn benchmark_padded_convolution_branching(kernel: &GaussianKernel, xic: &Array2<f32>) -> Array2<f32> {
    let (n_fragments, n_points) = xic.dim();
    let kernel_size = kernel.kernel_array.len();
    let half_kernel = kernel_size / 2;
...
pub fn run_convolution_benchmark() -> Vec<BenchmarkResult> {
    ...
    let implementations: Vec<(String, ConvolutionFunction)> = vec![
        ...
        ("Branching".to_string(), benchmark_padded_convolution_branching),
    ...
    for result in &results {
        println!("{} implementation: {:.4} seconds (speedup: {:.2}x)", 
