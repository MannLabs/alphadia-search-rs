...
fn test_natural_backend_selection() {
    ...
    #[cfg(target_arch = "aarch64")]
    {
        if NEON.is_available() {
            assert_eq!(selected_backend, "neon", "Should select NEON on aarch64 with NEON support");
            println!("✓ NEON backend correctly selected on aarch64");
        } else {
            assert_eq!(selected_backend, "scalar", "Should fall back to scalar when NEON unavailable");
    ...
    {
        assert_eq!(selected_backend, "scalar", "Should select scalar backend on non-aarch64");
...
fn test_neon_backend_override() {
    ...
    {
        let result = set_backend("neon");
        assert!(result.is_err(), "Should fail to set NEON backend on non-aarch64");
...
fn test_optimal_vs_current_backend() {
    ...
    #[cfg(target_arch = "aarch64")]
    {
        if NEON.is_available() {
            ...
            assert_eq!(get_current_backend(), "scalar", "Current should respect override");
            assert_eq!(get_optimal_simd_backend(), "neon", "Optimal should ignore override");
...
fn test_invalid_backend_error_handling() {
    ...
    {
        if NEON.is_available() {
            assert_eq!(backend, "neon", "Should maintain natural selection after invalid backend");
        } else {
            assert_eq!(backend, "scalar", "Should maintain natural selection after invalid backend");
        }
    }
