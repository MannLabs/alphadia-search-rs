...
mod tests {
    ...
    fn assert_xic_equivalence<T1: DIADataTrait, T2: DIADataTrait>(
        ...
        let xic1 = test_xic_slice_generic(dia_data1, obs_idx, cycle_start, cycle_stop, tolerance, test_mz);
        let xic2 = test_xic_slice_generic(dia_data2, obs_idx, cycle_start, cycle_stop, tolerance, test_mz);
        ...
        F: Fn(&crate::dia_data::DIAData, &crate::dia_data_next_gen::DIADataNextGen),
    {
        ...
        assert_eq!(old_dia_data.num_observations(), new_dia_data.num_observations());
    ...
    fn test_fill_xic_slice_equivalence_basic() {
        ...
        let isolation_lower_mz = [100.0f32, 100.0, 100.0, 200.0, 200.0, 200.0, 300.0, 300.0, 300.0];
        let isolation_upper_mz = [150.0f32, 150.0, 150.0, 250.0, 250.0, 250.0, 350.0, 350.0, 350.0];
    ...
    fn test_fill_xic_slice_equivalence_edge_cases() {
        ...
        for i in 0..6 {
            assert!((old_xic[i] - new_xic[i]).abs() < 1e-6,
                "XIC mismatch for non-sequential cycles at cycle {}: old={}, new={}", 
                i, old_xic[i], new_xic[i]);
        }
    ...
    fn test_fill_xic_slice_equivalence_precision() {
        ...
        let peak_mz = [
            150.000000f32, 150.000001, 150.000002,  // cycle 0
            150.000003, 150.000004, 150.000005,     // cycle 1
            150.000006, 150.000007, 150.000008,     // cycle 2
        ];
        ...
        for tolerance in tolerances {
            ...
            for i in 0..3 {
                assert!((old_xic[i] - new_xic[i]).abs() < 1e-10,
                    "High precision XIC mismatch at tolerance={}, cycle={}: old={}, new={}", 
                    tolerance, i, old_xic[i], new_xic[i]);
            }
        }
    }
        ...
        for obs in 0..num_obs {
            for cycle in 0..cycles_per_obs {
                ...
                for peak in 0..peaks_per_spectrum {
                    ...
                    peak_intensity.push(1000.0 + (obs * cycles_per_obs + cycle) as f32 * 100.0 + peak as f32);
                }
        ...
        for (cycle_start, cycle_stop, test_mz) in test_cases {
            ...
            for i in 0..xic_size {
                assert!((old_xic[i] - new_xic[i]).abs() < 1e-6,
                    ...
                    cycle_start, cycle_stop, i, old_xic[i], new_xic[i]);
            }
        }
    }
